...
string encrypt(string message)
{
    ...
    for(int i=0; i<message.length(); i+=2)    // i is incremented by 2 inorder to group by two two characters
    {
        ...
        if( x1 == x2 )
        {
            ctext.append(1, mat[x1][(y1+1)%5]);//append character 1 time
            ctext.append(1, mat[x2][(y2+1)%5]);//append character 1 time
        }
        else if( y1 == y2 )
        {
            ctext.append(1, mat[ (x1+1)%5 ][ y1 ]);//append character 1 time
            ctext.append(1, mat[ (x2+1)%5 ][ y2 ]);//append character 1 time
        }
        else
        {
            ctext.append(1, mat[ x1 ][ y2 ]);//append character 1 time
            ctext.append(1, mat[ x2 ][ y1 ]);//append character 1 time
        }
    }
    return ctext;
}
